...
drive routing, so the reported traffic fractions are upper bounds under an
ideal semantic router (Section~\ref{sec:practical-implications}).
Config~CD+schema achieves 89.00\% at 7B (Section~\ref{sec:cd-schema}), which
falls within the lower BFCL~v4 Python Simple AST leaderboard band between
GPT-5-mini (78.75\%) and GPT-4.1 (91.00\%). It is the first constrained
configuration in this evaluation to reach that band while retaining structural
...
Against the cascade proxy criterion (a single consumer GPU and a local tier
handling $p \geq 0.70$ of single-call traffic under an ideal semantic router),
five configurations meet the proxy threshold.
...
Finally, the cross-family evidence in this thesis is a targeted external-validity check rather than a full replication. Config~CD and CD+FT-aligned are evaluated across contrast checkpoints, but the quantized and agentic configurations remain concentrated on Qwen~2.5. Extending AWQ INT4, quantized fine-tuned variants, and $\tau$-bench evaluations across the same contrast families would test whether the deployment conclusions hold beyond the primary model family.
