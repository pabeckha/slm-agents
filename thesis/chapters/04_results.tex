...
for Config~B and Config~CD across all four Qwen~2.5 Instruct sizes.

\begin{table}[ht]
\centering
\caption{BFCL~v4 \texttt{simple\_python} AST accuracy for Config~B and
Config~CD across Qwen~2.5 Instruct model sizes (400 test cases).}
\label{tab:model-size}
\begin{tabular}{lrrr}
\toprule
Model size & Config~B & Config~CD & CD gain (pp) \\
\midrule
...
Config~B accuracy is near-zero across all model sizes, ranging from 1.50\%
...
Config~CD accuracy scales consistently with model size. The 0.5B model
achieves 51.50\%, the 1.5B achieves 62.25\%, the 3B achieves 64.75\%, and
the 7B achieves 72.75\%. The CD gain over Config~B also increases with model
...
the format barrier. The 0.5B model already reaches 51.50\% under Config~CD,
...
evaluated on Qwen~2.5 7B-Instruct-AWQ under Config~B and Config~CD+Q.

\begin{table}[ht]
\centering
\caption{BFCL~v4 AST accuracy on harder categories for Config~B and
Config~CD+Q (Qwen~2.5 7B-Instruct-AWQ, 200 test cases per category).}
\label{tab:bfcl-extended}
\begin{tabular}{lrr}
\toprule
Category & Config~B & Config~CD+Q \\
\midrule
