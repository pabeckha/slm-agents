...
Table~\ref{tab:bfcl-extended} reports results for the
\texttt{multiple\_function} and \texttt{parallel\_function} BFCL categories,
evaluated on Qwen~2.5 7B-Instruct-AWQ under Base and CD+Q.

\begin{table}[ht]
\centering
\caption{BFCL~v4 AST accuracy on harder categories for Base and
CD+Q (Qwen~2.5 7B-Instruct-AWQ, 200 test cases per category).}
\label{tab:bfcl-extended}
\begin{tabular}{lrr}
\toprule
Category & Base & CD+Q \\
\midrule
\texttt{multiple\_function}  & 0.0\%\ (0/200) & 0.0\%\ (0/200) \\
\texttt{parallel\_function}  & 0.0\%\ (0/200) & 0.0\%\ (0/200) \\
\bottomrule
\end{tabular}
\end{table}

Both configurations produce 0.0\% accuracy on both categories. Inspection of
the raw predictions shows that the pipeline outputs an empty list for every
test case. The cause is architectural: the two-stage pipeline is designed for
single-function selection in Stage~1 and single-call argument extraction in
Stage~2. The \texttt{parallel\_function} category requires generating multiple
function calls simultaneously, which the pipeline cannot produce because
Stage~2 is constrained to a single JSON object conforming to one function
schema. The \texttt{multiple\_function} category requires selecting one
function from several candidates; while \texttt{guided\_choice} supports this
in principle, the pipeline does not produce output in the format expected by
the BFCL evaluator for this category.

This negative result defines the boundary of the current pipeline
architecture. The optimization techniques evaluated in this thesis are all
applied within the single-call pipeline design. None address the structural
limitation of producing multiple simultaneous function calls. Extending the
pipeline to support parallel function calling would require a different
Stage~2 constraint: a JSON array of objects each conforming to a separate
function schema. This extension is left for future work.
