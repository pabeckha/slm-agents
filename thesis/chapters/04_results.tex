...
cases). CD was evaluated at 0.5B--3B only; the 7B CD condition was not run
for this category. CD+FT-aligned was evaluated at all four sizes.}
...
CD alone scales from 42.0\% at 0.5B to 62.0\% at 3B. CD+FT-aligned improves
over CD by 13.5~pp at 0.5B and 7.5~pp at 1.5B, but the gap closes to within
noise at 3B (60.5\% vs.\ 62.0\%). At 7B, CD+FT-aligned reaches 70.5\%.
The pattern mirrors the simple\_python sweep: fine-tuning helps most at
smaller sizes where the model has limited capacity to infer the required
format from the prompt alone.
