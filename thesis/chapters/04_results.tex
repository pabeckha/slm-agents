...
Table~\ref{tab:model-size} reports BFCL~v4 \texttt{simple\_python} accuracy
for Base and CD across all four Qwen~2.5 Instruct sizes.

\begin{table}[ht]
\centering
\caption{BFCL~v4 \texttt{simple\_python} AST accuracy for Base and
CD across Qwen~2.5 Instruct model sizes (400 test cases).}
\label{tab:model-size}
\begin{tabular}{lrrr}
\toprule
Model size & Base & CD & CD gain (pp) \\
\midrule
0.5B & 3.50\%\ (14/400)  & 51.50\%\ (206/400) & $+$48.0 \\
1.5B & 4.75\%\ (19/400)  & 62.25\%\ (249/400) & $+$57.5 \\
3B   & 2.75\%\ (11/400)  & 64.75\%\ (259/400) & $+$62.0 \\
7B   & 1.50\%\ (6/400)   & 72.75\%\ (291/400) & $+$71.25 \\
\bottomrule
\end{tabular}
\end{table}
...
CD accuracy scales consistently with model size. The 0.5B model
achieves 51.50\%, the 1.5B achieves 62.25\%, the 3B achieves 64.75\%, and
the 7B achieves 72.75\%. The CD gain over Base also increases with model
size, from 48.0~pp at 0.5B to 71.25~pp at 7B. This pattern reflects two
compounding factors: larger models produce fewer argument value errors once
format compliance is enforced, and they benefit more from constrained decoding
because a larger fraction of their latent capability is unlocked by removing
the format barrier. The 0.5B model already reaches 51.50\% under CD,
approaching GPT-5-mini (59.92\%), while the 7B model exceeds GPT-4.1 (72.67\%).
